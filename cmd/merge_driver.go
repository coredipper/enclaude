...
// readManifests reads and parses the ancestor, ours, and theirs manifests.
func readManifests(ancestorFile, oursFile, theirsFile string) (ancestor, ours, theirs sealstore.Manifest, err error) {
	ancestorData, _ := os.ReadFile(ancestorFile) // may not exist
	oursData, err := os.ReadFile(oursFile)
	if err != nil {
		return ancestor, ours, theirs, fmt.Errorf("reading ours: %w", err)
	}
	theirsData, err := os.ReadFile(theirsFile)
	if err != nil {
		return ancestor, ours, theirs, fmt.Errorf("reading theirs: %w", err)
	}
	...
	if err := json.Unmarshal(oursData, &ours); err != nil {
		return ancestor, ours, theirs, fmt.Errorf("parsing ours manifest: %w", err)
	}
	if err := json.Unmarshal(theirsData, &theirs); err != nil {
		return ancestor, ours, theirs, fmt.Errorf("parsing theirs manifest: %w", err)
	}

	return ancestor, ours, theirs, nil
}

// resolveFileConflict resolves the merge for a single file within the manifest.
func resolveFileConflict(
	path string,
	cfg *config.Config,
	identity *age.X25519Identity,
	objStore *sealstore.ObjectStore,
	ancestor, ours, theirs sealstore.Manifest,
	...
	if strategy == merge.JSONLDedup && filepath.Base(path) == "sessions-index.json" {
		return oursEntry, fmt.Errorf("refusing to merge %s with jsonl_dedup (sessions-index.json is JSON, not JSONL; rule %q matched). Fix: change the strategy to 'sessions_index' in seal.toml, or run 'enclaude upgrade'", path, winningPattern)
	}
...
// writeMergedManifest writes the merged manifest back to disk.
func writeMergedManifest(merged sealstore.Manifest, oursFile, sealDir string) error {
	...
	if err := os.WriteFile(sealDir+"/manifest.json", mergedData, 0600); err != nil {
...
func mergeManifests(ancestorFile, oursFile, theirsFile string) error {
	...
	// Read manifests
	ancestor, ours, theirs, err := readManifests(ancestorFile, oursFile, theirsFile)
	if err != nil {
		return err
	}
	...
	for path := range allPaths {
		...
		resolvedEntry, err := resolveFileConflict(
			path, cfg, identity, objStore,
			ancestor, ours, theirs,
			oursEntry, theirsEntry,
		)
		if err != nil {
			return err
		}
	...
	return writeMergedManifest(merged, oursFile, cfg.Seal.SealDir)
}
